...
def calculateOffense(finder,team,opponent):
    ...
    FTMade = (finder.getFTMade(team) + finder.getOppFTMade(opponent)) / 2
    relativeStrength = finder.getStrengthOfSchedule(team) / finder.getMaxStrengthOfSchedule()
    oppRelativeStrength = finder.getStrengthOfSchedule(opponent) / finder.getMaxStrengthOfSchedule()
    ...
    ##Account for FT ##FIXME
    points += FTMade

    ##Account for strength of schedule
    points += (points * (relativeStrength - oppRelativeStrength))
